...
const INTENTS: IntentDef[] = [
  ...
  FILTER(CONTAINS(LCASE(?label), "${quoteLiteral((b.term ?? "").toLowerCase())}"))`,
    cypher: (b) => `MATCH (n) WHERE toLower(n.name) CONTAINS "${quoteLiteral((b.term ?? "").toLowerCase())}" RETURN n LIMIT 25`,
...
/** Escapes text for a double-quoted SPARQL or Cypher string literal. */
...
function extractBindings(intentId: string, norm: string): NlqBindings {
  ...
  if (intentId === "node-lookup") {
    // Same verbs as the intent pattern, so a matched question always yields a term.
    const m = norm.match(/(?:show|find|look ?up|get|tell me about)(?: me)?(?: the)?(?: node)? (.+)/);
    if (m) return { term: m[1].trim() };
    return { term: norm.slice(0, 60) };
  }
  return {};
}

export async function translate(question: string): Promise<NlqResult> {
  ...
  for (const intent of INTENTS) {
    ...
    const bindings = extractBindings(intent.id, norm);
    const sparql =
      `# intent:${intent.id}\n` +
  ...
  // Fallback to pluggable LLM Gateway when available
  try {
    const llmRes = await llmGateway.generateSparql(question);
    if (llmRes) {
      return {
        recognized: true,
        intent: "llm-generated",
        sparql: `# intent:llm-generated\n# provider:${llmRes.providerId}\n# model:${llmRes.model}\n${llmRes.sparql}`,
        explanation: `${llmRes.explanation} (Generated via ${llmRes.providerId} [${llmRes.model}])`,
        grounding: { classes: [], predicates: [] },
      };
    }
  } catch (err) {
    ...
    // Fall through to suggestions on provider error
  }
  ...
  return {
    ...
    explanation:
      "No intent matched. The simulator recognizes ~12 ontology-grounded question shapes — try one of the suggestions or configure an LLM provider.",
  };
}
...
export async function executeGenerated(
  ...
  // The text comes from the client, so it gets the same comment- and
  // literal-aware read-only check as /api/sparql.
  ...
  if (!intentMatch || (intentMatch[1] !== "llm-generated" && !INTENTS.some((i) => i.id === intentMatch[1]))) {
    throw new Error(
